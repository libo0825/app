...
export default {
  ...
  mounted() {
    let id = sessionStorage.getItem("changeId");
    this.axios.post("/api/back/module/post", { id }).then(data => {
      console.log(data.data[0]);
      this.msg[0].data = data.data[0].title;
      this.msg[1].data = data.data[0].url;
      this.msg[2].data = data.data[0].type;
      this.msg[3].data = data.data[0].backorfont;
      this.msg[4].data = data.data[0].getparams;
      this.msg[5].data = data.data[0].sendparams;
    });
  }
};
</script>
